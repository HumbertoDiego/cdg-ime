# Ciência de Dados Geoespaciais IME

Aulas de Ciência de Dados Geoespaciais ministradas para o 3ª ano do Curso de Engenharia Cartográfica do Instituto Militar de Engenharia - Rio de Janeiro/RJ

VISÃO GERAL DA DISCIPLINA:

<img src="media/imgs/Ciência de dados geoespaciais.png">

SUMÁRIO:

- [Prospecção de dados](https://github.com/HumbertoDiego/cdg-ime/blob/main/Prospec%C3%A7%C3%A3o.ipynb)
- [Pré Processamento](https://github.com/HumbertoDiego/cdg-ime/blob/main/PreProcessamento.ipynb)
- [Mineração](https://github.com/HumbertoDiego/cdg-ime/blob/main/Minera%C3%A7%C3%A3o.ipynb)

REQUISITOS:

- [uv](https://docs.astral.sh/uv/getting-started/installation/)
- `uv python install 3.14`
- `uv sync`
- Para ativar no terminal:

  - `./.venv/Scripts/activate.ps1 # troque .ps1 dependendo do SO ou terminal`
- Feche o terminal e, se necessário, reinicie o VSCode
- Para ativar neste notebook:

  - `VSCode` > `Select Kernel` > `Python Environments...` > `cdg-ime (3.14.4)(Python 3.14.4) .venv\Scripts\python.exe`


<!-- 
git add * ; git commit -m "aula update"; git push cgd main
jupyter nbconvert --to slides Prospecção.ipynb --TagRemovePreprocessor.remove_input_tags="hide_input" --post serve
 -->